...
import static net.uk.onetransport.android.modules.bitcarriersilverstone.provider.BcsContract.BitCarrierSilverstoneDataTravelSummary;
...
//            "net.uk.onetransport.android.modules.bitcarriersilverstone.sync.METAVECTORS";
    private static final String EXTRAS_DATA_TRAVEL_SUMMARIES =
            "net.uk.onetransport.android.modules.bitcarriersilverstone.sync.DATA_TRAVELTIMES";
...
//                BitCarrierSilverstoneConfigTravelTime.TABLE_NAME);
        TRAVEL_SUMMARY_URI = Uri.withAppendedPath(AUTHORITY_URI,
                BitCarrierSilverstoneDataTravelSummary.TABLE_NAME);
...
//        providerModules.add(this);
        ...
        uriMatcher.addURI(authority, BitCarrierSilverstoneDataTravelSummary.TABLE_NAME, TRAVEL_SUMMARIES);
        providerModules.add(this);
        TRAVEL_SUMMARY_ID = providerModules.size();
        uriMatcher.addURI(authority, BitCarrierSilverstoneDataTravelSummary.TABLE_NAME + "/#",
                TRAVEL_SUMMARY_ID);
        providerModules.add(this);
...
//        }
        if (match == TRAVEL_SUMMARIES) {
            return mimeDirPrefix + BitCarrierSilverstoneDataTravelSummary.TABLE_NAME;
        }
        if (match == TRAVEL_SUMMARY_ID) {
            return mimeItemPrefix + BitCarrierSilverstoneDataTravelSummary.TABLE_NAME;
        }
...
//        }
        if (match == TRAVEL_SUMMARIES) {
            id = sqLiteDatabase.insert(BitCarrierSilverstoneDataTravelSummary.TABLE_NAME, null, contentValues);
...
//        }
        if (match == TRAVEL_SUMMARIES) {
            Cursor cursor = sqLiteDatabase.query(BitCarrierSilverstoneDataTravelSummary.TABLE_NAME, projection,
                    selection, selectionArgs, null, null, sortOrder);
        ...
        if (match == TRAVEL_SUMMARY_ID) {
            Cursor cursor = sqLiteDatabase.query(BitCarrierSilverstoneDataTravelSummary.TABLE_NAME, projection,
                    BitCarrierSilverstoneDataTravelSummary._ID + "=?", new String[]{uri.getLastPathSegment()}, null, null,
                    sortOrder);
            cursor.setNotificationUri(contentResolver, TRAVEL_SUMMARY_URI);
            return cursor;
        }
...
//        }
        if (match == TRAVEL_SUMMARIES) {
            int rows = sqLiteDatabase.update(BitCarrierSilverstoneDataTravelSummary.TABLE_NAME, values, selection,
                    selectionArgs);
            contentResolver.notifyChange(TRAVEL_SUMMARY_URI, null);
            return rows;
        }
        if (match == TRAVEL_SUMMARY_ID) {
            int rows = sqLiteDatabase.update(BitCarrierSilverstoneDataTravelSummary.TABLE_NAME, values,
                    BitCarrierSilverstoneDataTravelSummary._ID + "=?", new String[]{uri.getLastPathSegment()});
            contentResolver.notifyChange(TRAVEL_SUMMARY_URI, null);
            return rows;
        }
        if (match == CONFIG_VECTORS) {
...
//        }
        if (match == TRAVEL_SUMMARIES) {
            rows = sqLiteDatabase.delete(BitCarrierSilverstoneDataTravelSummary.TABLE_NAME, selection,
                    selectionArgs);
            contentResolver.notifyChange(TRAVEL_SUMMARY_URI, null);
        }
        if (match == CONFIG_VECTORS) {
...
//                }
//            }
            if (extras.getBoolean(EXTRAS_DATA_TRAVEL_SUMMARIES, false)) {
                try {
    ...
    public static void refresh(Context context,
                               ...
                               boolean nodes,
//                               boolean routes,
                               boolean configVectors,
//                               boolean zones,
//                               boolean metavectors,
                               boolean dataTravelSummaries) {
...
//        settingsBundle.putBoolean(EXTRAS_METAVECTORS, metavectors);
        settingsBundle.putBoolean(EXTRAS_DATA_TRAVEL_SUMMARIES, dataTravelSummaries);
