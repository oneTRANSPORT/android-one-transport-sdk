...
public class NorthantsTrafficScootDeleteTest extends OneTransportTest {
    ...
    public void startAsync(DougalCallback dougalCallback) {
        ((RunnerFragment) dougalCallback).setCurrentTest("NORTHANTS traffic scoot delete");
        dougalCallback.getResponse(null, new Exception("Not implemented"));
    }

    private void deleteTrafficScoot(RunnerTask runnerTask) throws Exception {
        runnerTask.setCurrentTest("NORTHANTS traffic scoot delete");
        Context context = runnerTask.getContext();
        NorthantsContentHelper.deleteFromProvider(context,
                NorthantsContentHelper.DATA_TYPE_TRAFFIC_SCOOT);
        TrafficScoot[] trafficScoots = NorthantsContentHelper.getTrafficScoots(context);
        if (trafficScoots.length == 0) {
            runnerTask.report("NORTHANTS traffic scoot delete ... PASSED.", COLOUR_PASSED);
            return;
        }
        runnerTask.report("NORTHANTS traffic scoot delete ... FAILED.", COLOUR_FAILED);
    }
}
