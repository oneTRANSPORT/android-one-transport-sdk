 ...
 */
package net.uk.onetransport.android.test.onetransporttest.tests.northants.carpark;
...
import net.uk.onetransport.android.county.northants.carparks.CarParkRetrieverLoader;
import net.uk.onetransport.android.county.northants.generic.RetrieverResult;
import net.uk.onetransport.android.test.onetransporttest.RunnerFragment;
import net.uk.onetransport.android.test.onetransporttest.RunnerTask;
import net.uk.onetransport.android.test.onetransporttest.tests.OneTransportTest;

public class NorthantsCarParkRetrieveTest extends OneTransportTest
    ...
    public void startAsync(DougalCallback dougalCallback) {
        ((RunnerFragment) dougalCallback).setCurrentTest("NORTHANTS get car park array");
    ...
    public void onLoadFinished(Loader<RetrieverResult<CarPark>> loader, RetrieverResult<CarPark> data) {
        if (data.getExceptions().size() > 0 || data.getContent() == null) {
            dougalCallback.getResponse(null, new Throwable("NORTHANTS car park array error"));
        } else {
    ...
    private void getCarParkArray(RunnerTask runnerTask) throws Exception {
        runnerTask.setCurrentTest("NORTHANTS get car park array");
        CarPark[] carParks = new CarParkRetriever(getContext()).retrieve();
        if (carParks == null || carParks.length == 0) {
            Log.i("NorthantsCarPark", "Error no data in feed");
            runnerTask.report("NORTHANTS get car park array ... FAILED.", COLOUR_FAILED);
        } else {
            runnerTask.report("NORTHANTS get car park array ... PASSED.", COLOUR_PASSED);
        }
    }
}
