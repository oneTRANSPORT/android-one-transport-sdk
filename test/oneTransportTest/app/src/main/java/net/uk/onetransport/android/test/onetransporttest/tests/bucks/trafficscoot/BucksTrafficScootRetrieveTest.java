 ...
 */
package net.uk.onetransport.android.test.onetransporttest.tests.bucks.trafficscoot;
...
import net.uk.onetransport.android.county.bucks.trafficscoot.TrafficScootRetrieverLoader;
import net.uk.onetransport.android.test.onetransporttest.RunnerFragment;
import net.uk.onetransport.android.test.onetransporttest.RunnerTask;
import net.uk.onetransport.android.test.onetransporttest.tests.OneTransportTest;

public class BucksTrafficScootRetrieveTest extends OneTransportTest
    ...
    public void startAsync(DougalCallback dougalCallback) {
        ((RunnerFragment) dougalCallback).setCurrentTest("BUCKS get traffic scoot array");
    ...
    public void onLoadFinished(Loader<RetrieverResult<TrafficScoot>> loader,
        ...
        if (data.getExceptions().size() > 0 || data.getContent() == null) {
            dougalCallback.getResponse(null, new Throwable("Bucks traffic scoot array error"));
        } else {
    ...
    private void getTrafficScootArray(RunnerTask runnerTask) throws Exception {
        runnerTask.setCurrentTest("BUCKS get traffic scoot array");
        TrafficScoot[] trafficScoots = new TrafficScootRetriever(getContext()).retrieve();
        if (trafficScoots == null || trafficScoots.length == 0) {
            Log.i("BucksTrafficScoot", "Error no data in feed");
            runnerTask.report("BUCKS get traffic scoot array ... FAILED.", COLOUR_FAILED);
        } else {
            runnerTask.report("BUCKS get traffic scoot array ... PASSED.", COLOUR_PASSED);
        }
    }
}