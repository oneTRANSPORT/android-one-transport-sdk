package net.uk.onetransport.android.test.onetransporttest.tests.northants.trafficqueue;
...
public class NorthantsTrafficQueueRetrieveTest extends OneTransportTest
    ...
    public void startAsync(DougalCallback dougalCallback) {
        ((RunnerFragment) dougalCallback).setCurrentTest("NORTHANTS get traffic queue array");
    ...
    public void onLoadFinished(Loader<RetrieverResult<TrafficQueue>> loader,
        ...
        if (data.getExceptions().size() > 0 || data.getContent() == null) {
            dougalCallback.getResponse(null, new Throwable("NORTHANTS traffic queue array error"));
        } else {
    ...
    private void getTrafficQueueArray(RunnerTask runnerTask) throws Exception {
        runnerTask.setCurrentTest("NORTHANTS get traffic queue array");
        TrafficQueue[] trafficQueues = new TrafficQueueRetriever(getContext()).retrieve();
        if (trafficQueues == null || trafficQueues.length == 0) {
            runnerTask.report("NORTHANTS get traffic queue array ... FAILED.", COLOUR_FAILED);
        } else {
            runnerTask.report("NORTHANTS get traffic queue array ... PASSED.", COLOUR_PASSED);
        }
    }
}
