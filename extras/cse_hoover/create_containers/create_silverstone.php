...
unset($result);
$lines = file('clearview_total.txt');
foreach ($lines as $line) {
  list($days, $hours, $minutes, $direction, $num) = explode('|', $line);
  $result[$days][$hours][($minutes / 15)][trim($direction)] += $num;
}
mkdir("$root/total");
$occupancy = 0;
foreach ($result as $day => $hours) {
  mkdir("$root/total/day$day");
  $day_total_in      = 0;
  $day_total_out     = 0;
  $day_min_in        = 9999999;
  $day_min_out       = 9999999;
  ...
  $day_max_occupancy = -9999999;
  foreach ($hours as $hour => $minutes) {
    mkdir("$root/total/day$day/hour$hour");
    $hour_total_in      = 0;
    $hour_total_out     = 0;
    $hour_min_in        = 9999999;
    $hour_min_out       = 9999999;
    ...
    foreach ($minutes as $minute => $values) {
      ...
      switch ($minute) {
        case 0:
          mkdir("$root/total/day$day/hour$hour/mins00-14");
          file_put_contents("$root/total/day$day/hour$hour/mins00-14/cin", $json);
          break;
        case 1:
          mkdir("$root/total/day$day/hour$hour/mins15-29");
          file_put_contents("$root/total/day$day/hour$hour/mins15-29/cin", $json);
          break;
        case 2:
          mkdir("$root/total/day$day/hour$hour/mins30-44");
          file_put_contents("$root/total/day$day/hour$hour/mins30-44/cin", $json);
          break;
        case 3:
          mkdir("$root/total/day$day/hour$hour/mins45-59");
          file_put_contents("$root/total/day$day/hour$hour/mins45-59/cin", $json);
          break;
      }
    }
    // Hourly average.
    mkdir("$root/total/day$day/hour$hour/hour");
    $json = '{"total-in":'      . (int)$hour_total_in
          . ',"total-out":'     . (int)$hour_total_out
          . ',"occupancy-end":' . (int)$occupancy
          . ',"min-in":'        . (int)$hour_min_in
          ...
          . ',"max-occupancy":' . (int)$hour_max_occupancy
          . "}\n";
    file_put_contents("$root/total/day$day/hour$hour/hour/cin", $json);
    $day_total_in  += $hour_total_in;
    $day_total_out += $hour_total_out;
    if ($day_min_in        > $hour_min_in)        { $day_min_in        = $hour_min_in; }
    if ($day_min_out       > $hour_min_out)       { $day_min_out       = $hour_min_out; }
  ...
  $json = '{"total-in":'      . (int)$day_total_in
        ...
        . ',"max-occupancy":' . (int)$day_max_occupancy
        . "}\n";
  file_put_contents("$root/total/day$day/day/cin", $json);
}
